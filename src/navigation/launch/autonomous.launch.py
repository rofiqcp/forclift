...
This launch stays LOCALIZATION-ONLY with respect to the saved occupancy map:
slam_toolbox, map_saver, map.launch.py and mapping_runtime.launch.py are never
started.  nav2_map_server is the only /map source.  A LiDAR scan-matching node is started in ODOMETRY-ONLY mode (no /map and no TF
publication) as a diagnostic/fallback comparison only. EKF remains the sole
owner of odom -> base_footprint and fuses independent wheel/ESC velocity + IMU gyro.
...
def generate_launch_description():
    ...
    # Fail-open: drivers start after a short delay independent of the transport
    # gate. respawn=True owns reconnection on USB reconnect. This mirrors the
    # mapping_runtime bring-up and prevents the preflight/strict-gate chain from
    # holding the entire sensor+foundation graph when CP210x flaps.
    ...
    start_navigation_aux_after_both_costmaps = _success_only_exit(
        both_costmaps_ready_gate, [
                ...
                LogInfo(msg=(
                    "[NAV2-AUX] READY: both costmaps verified; auxiliary lifecycle manager "
                    "was already started once by the V60 foundation and remains the sole owner."
                )),
            ], "both-costmaps-ready")
    ...
    yolo_detection_cfg = os.path.join(yolo_share, "config", "yolo_detection.yaml")
    yolo = Node(
        ...
        parameters=[yolo_detection_cfg, {
            ...
            "use_tensorrt": ParameterValue(
                LaunchConfiguration("resolved_use_tensorrt"), value_type=bool
            ),
            "use_cuda": True,
            "require_cuda": ParameterValue(
                LaunchConfiguration("yolo_require_cuda"), value_type=bool
            ),
            "max_inference_fps": ParameterValue(
                LaunchConfiguration("yolo_max_inference_fps"), value_type=float
            ),
    ...
    # V60 CRITICAL DECOUPLING: the navigation foundation must never depend on
    # perception cleanup exit status.  V59 still used perception_preflight as
    # the root event for RViz, AMCL and Nav2; any abnormal cleanup exit could
    # therefore make RViz and Smac disappear together.  Start the foundation
    # from its own bounded timer. Perception remains a completely independent
    # branch and cannot suppress localization/planning availability.
    start_foundation_after_preflight = TimerAction(period=0.30, actions=[
                robot_state,
                visual_robot_state,
                visual_base_bridge,
                joint_state,
                ...
                TimerAction(period=1.50, actions=[rviz]),
                esc,
                ekf,
                ...
                smoother,
                collision,
                # V60 EXACTLY-ONCE LIFECYCLE OWNERSHIP.  Availability is no
                # longer chained through one-shot sensor/costmap gates.  A
                # temporary PRE-AMCL failure must not leave AMCL, Controller or
                # BT Navigator permanently UNCONFIGURED.  Motion remains
                # fail-closed through autonomy_health_manager + command guards.
                TimerAction(period=0.80, actions=[localization_lifecycle]),
                TimerAction(period=1.00, actions=[lifecycle_navigation_local]),
                TimerAction(period=1.20, actions=[lifecycle_navigation_aux]),
    ...
    return LaunchDescription(args + [
        ...
        camera_ready_notice,
        # RVIZ-ONLY FIX: RViz is started directly from the autonomous foundation.
        # The old RobotModel exit-gate handler is intentionally not registered.
        # NOTE: sensor_preflight (destructive CP210x recovery) stays disabled.
        # Perception cleanup is independent; it no longer owns or gates the
        # navigation foundation, RViz, AMCL or Nav2 startup.
        start_perception_after_cleanup,

        # Sole direct hardware root: remove stale Astra/YOLO ownership first.
        perception_preflight,
    ])
