...
bool SerialPort::set_motor_power(bool enabled)
{
  ...
  // Many YDLiDAR USB adapters enable motor with RTS/DTR deasserted (active-low).
  int lines = TIOCM_RTS | TIOCM_DTR;
  return ioctl(fd_, enabled ? TIOCMBIC : TIOCMBIS, &lines) == 0;
}
...
bool LiDARMotor::probe_for_sync(int wait_ms)
{
  ...
  while (std::chrono::steady_clock::now() < deadline) {
    ...
    for (size_t i = 1; i < chunk.size(); ++i) {
      if (chunk[i - 1] == 0xAA && chunk[i] == 0x55) {
        ++sync;
      }
    }
    if (sync >= 1) {
...
bool LiDARMotor::start_scan(int wait_ms)
{
  ...
  static const std::vector<std::vector<uint8_t>> start_cmds = {
    ...
    {kCmdStartTmini[0], kCmdStartTmini[1]},
  };
...
: Node("lidar_node", options)
{
  ...
  scan_frequency_ = get_parameter("scan_publish_rate").as_double();
  ...
  RCLCPP_INFO(get_logger(), "Scan frequency: %.1f Hz", scan_frequency_);
