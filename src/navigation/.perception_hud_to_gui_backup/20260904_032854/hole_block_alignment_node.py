...
def draw_hud(canvas, m, max_residual):
    ...
    # Camera/fork target box requested for the 1280x720 display.
    cv2.rectangle(canvas, (sx1, sy1), (sx2, sy2), (0, 0, 255), 2, cv2.LINE_AA)
    cv2.line(canvas, (spx, min(sy1, spy)), (spx, 0), (0, 0, 220), 1, cv2.LINE_AA)
    draw_marker(canvas, (spx, spy), (0, 0, 255), "cross", 10)
    cv2.putText(canvas, "SETPOINT", (sx1 + 4, max(18, sy1 - 6)),
    ...
    if m.block_detected:
        ...
        if m.detected_block_box is not None:
            ...
            cv2.putText(canvas, "MIDDLE BLOCK", (bx1, max(18, by1 - 6)),
    ...
    st_color = (0, 255, 0) if m.status == "ALIGNED" else \
               (0, 255, 255) if m.status in ("ALIGNING", "NOT STABLE", "LOCKING HOLE PAIR") else (0, 0, 255)
    ...
    left_lines = [
        ...
        (f"Res: {w} x {h}   Target box: X[{sx1},{sx2}] Y[{sy1},{sy2}]", (255, 255, 255)),
        ("", None),
        ...
        (f"Hole gate  : {m.hole_gate_count}/{m.hole_gate_req}", (0, 255, 255)),
        (f"Middle block: {'VALID' if m.block_detected else 'NONE'}  candidates={m.candidate_count}", (0, 255, 0) if m.block_detected else (0, 0, 255)),
    ...
    right_lines = [
        ...
        (f"Target ctr : X {m.setpoint_x_px:.0f}px  Y {m.setpoint_y_px:.0f}px", (0, 0, 255)),
        (f"Target box : X[{m.setpoint_x_min_px:.0f},{m.setpoint_x_max_px:.0f}] Y[{m.setpoint_y_min_px:.0f},{m.setpoint_y_max_px:.0f}]", (0, 0, 255)),
        (f"Yaw offset : {fmt(m.setpoint_yaw_offset_deg, 2)} deg", (255, 255, 255)),
        ("", None),
        ("ALIGNMENT OUTPUT", (255, 255, 255)),
        (f"Lat err    : {fmt(m.filtered_error_lateral_m, 4, ' m') if is_num(m.filtered_error_lateral_m) else 'N/A m'}" + (" [EST]" if is_num(m.filtered_error_lateral_m) and not m.metric_error_valid else ""), (0, 255, 0) if is_num(m.filtered_error_lateral_m) else (150, 150, 150)),
