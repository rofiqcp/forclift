...
WS="${AGV_WS:-$HOME/ros}"
SRC="$WS/src"
fail(){ echo "[V56-BUILD] FAIL: $*" >&2; exit 2; }
info(){ echo "[V56-BUILD] $*"; }
...
chmod +x \
  ...
  "$SRC/navigation/tools/run_v56.sh" \
  "$SRC/navigation/tools/verify_v56_master_gui_robot_model.sh" || true

info 'V56 master-GUI/map/graph + RobotModel startup-race fix build'
...
bash "$SRC/navigation/tools/run_v56.sh" check
