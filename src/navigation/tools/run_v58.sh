...
info(){ printf '[V58-RUN] %s\n' "$*"; }
warn(){ printf '[V58-RUN] WARN: %s\n' "$*" >&2; }
fail(){ printf '[V58-RUN] FAIL: %s\n' "$*" >&2; exit 2; }
...
check_mode(){
  ...
  case "$mode" in
    ...
    acceptance|acceptance-goal)
      ...
      ;;
    *) fail 'usage: run_v58.sh {gui|map|autonomous|check|acceptance|acceptance-goal} [ROS launch arguments...]' ;;
  esac
  if [[ -n "$launch" ]]; then
    check_file "$launch"
  fi
...
if [[ "$MODE" == "check" ]]; then
  ...
  info 'PASS: GUI + map inflation preview + decoupled AMCL/perception + Smac/MPPI runtime paths are runnable.'
  exit 0
fi
...
ROS_LOG_DIR="$WS/log/runtime_v58_${MODE}_${STAMP}"
mkdir -p "$ROS_LOG_DIR"
export ROS_LOG_DIR
info "mode=$MODE"
