...
info(){ printf '[V59-RUN] %s\n' "$*"; }
warn(){ printf '[V59-RUN] WARN: %s\n' "$*" >&2; }
fail(){ printf '[V59-RUN] FAIL: %s\n' "$*" >&2; exit 2; }
...
check_mode(){
  ...
  case "$mode" in
    ...
    perception-only)
      ...
      ;;
    *) fail 'usage: run_v59.sh {gui|map|autonomous|check|acceptance|acceptance-goal|perception-check|perception-only} [ROS launch arguments...]' ;;
  esac
  if [[ -n "$launch" ]]; then
    check_file "$launch"
  fi
...
if [[ "$MODE" == "check" ]]; then
  ...
  info 'PASS: GUI + master perception topology + AMCL/costmap + Smac/MPPI runtime paths are runnable.'
  exit 0
fi
...
ROS_LOG_DIR="$WS/log/runtime_v59_${MODE}_${STAMP}"
mkdir -p "$ROS_LOG_DIR"
export ROS_LOG_DIR
info "mode=$MODE"
