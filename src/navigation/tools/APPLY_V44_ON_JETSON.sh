...
if ! pkg-config --exists Qt5Widgets 2>/dev/null; then
  echo '[V44] Qt5 development headers are required for the native C++ GUI.'
...
echo '[V44] 1/6 static source verification'
bash "$NAV/tools/verify_v44_humble_build_fix.sh" "$NAV"

echo '[V44] 2/6 clean navigation build state'
rm -rf "$WS/build/navigation" "$WS/install/navigation"
mkdir -p "$WS/log"
cd "$WS"

echo '[V44] 3/6 colcon build'
colcon build --symlink-install --event-handlers console_direct+

echo '[V44] 4/6 source new install and verify C++ executables'
...
for exe in agv_gui_cpp mapping_gui_cpp goal_pose_nav2_bridge scan_self_filter navigation_runtime_validator lidar_node imu_node; do
  ...
  echo "[V44] PASS executable: $exe"
done

echo '[V44] 5/6 launch-file syntax and package discovery'
...
cat <<'EOF'
[V44] COMPLETE: Humble/GCC11 build verifier + clean build + install check + tests passed.
[V44] Start:
  ros2 launch navigation gui.launch.py
[V44] Then set 2D Pose Estimate and test baseline:
  bash src/navigation/tools/validate_v43_navigation_live.sh
[V44] Click a Nav2 Goal, then verify an actual Smac path:
