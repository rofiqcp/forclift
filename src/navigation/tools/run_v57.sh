...
info(){ printf '[V57-RUN] %s\n' "$*"; }
warn(){ printf '[V57-RUN] WARN: %s\n' "$*" >&2; }
fail(){ printf '[V57-RUN] FAIL: %s\n' "$*" >&2; exit 2; }
...
check_mode(){
  ...
  case "$mode" in
    ...
    autonomous|auto)
      ...
      ;;
    *) fail 'usage: run_v57.sh {gui|map|autonomous|check} [ROS launch arguments...]' ;;
  esac
  check_file "$launch"
  if [[ -n "$bin" ]]; then
    check_bin "$bin"
...
if [[ "$MODE" == "check" ]]; then
  ...
  info 'PASS: master GUI + isolated map goals + AMCL/camera telemetry, Mapping, and Autonomous root launch paths are runnable.'
  exit 0
fi
...
ROS_LOG_DIR="$WS/log/runtime_v57_${MODE}_${STAMP}"
mkdir -p "$ROS_LOG_DIR"
export ROS_LOG_DIR
info "mode=$MODE"
