...
WS="${AGV_WS:-$HOME/ros}"
SRC="$WS/src"
fail(){ echo "[V59-BUILD] FAIL: $*" >&2; exit 2; }
info(){ echo "[V59-BUILD] $*"; }
...
chmod +x \
  ...
  "$SRC/RUN_ACCEPTANCE_V59.sh" "$SRC/RUN_GOAL_ACCEPTANCE_V59.sh" "$SRC/RUN_PERCEPTION_ACCEPTANCE_V59.sh" "$SRC/RUN_PERCEPTION_ONLY_V59.sh" \
  ...
  "$SRC/navigation/tools/run_v59.sh" "$SRC/navigation/tools/verify_v59_perception_master.sh" || true

info 'Building V59: restore uploaded-master perception topology + retain AMCL/costmap/Smac/MPPI fixes'
...
export AGV_WS="$WS"
bash "$SRC/navigation/tools/verify_v59_perception_master.sh"
...
bash "$SRC/navigation/tools/verify_v59_perception_master.sh"
bash "$SRC/navigation/tools/run_v59.sh" check

info 'PASS: V59 built; master perception topology and root launch paths validated.'
echo 'Autonomous:          bash src/RUN_AUTONOMOUS.sh'
echo 'GUI:                 bash src/RUN_GUI.sh'
echo 'Static/live check:   bash src/RUN_CHECK.sh'
echo 'Live acceptance:       bash src/RUN_ACCEPTANCE_V59.sh'
echo 'Perception acceptance: bash src/RUN_PERCEPTION_ACCEPTANCE_V59.sh'
echo 'Perception only test:  bash src/RUN_PERCEPTION_ONLY_V59.sh'
echo 'After setting Goal:    bash src/RUN_GOAL_ACCEPTANCE_V59.sh'
