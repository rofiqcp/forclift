...
class NavMapSwitchController(Node):
    ...
    def _validate_slot(self, slot):
        ...
        if slot not in (1, 2, 3):
            raise RuntimeError('slot map harus 1..3')
        if not yaml_path.is_file() or yaml_path.stat().st_size <= 0:
            raise RuntimeError(f'Map {slot} YAML belum tersedia: {yaml_path}')
        if not pgm_path.is_file() or pgm_path.stat().st_size <= 0:
            raise RuntimeError(f'Map {slot} PGM belum tersedia: {pgm_path}')
        text = yaml_path.read_text(encoding='utf-8', errors='replace')
        if f'map_{slot}.pgm' not in text:
            raise RuntimeError(f'Map {slot} YAML tidak menunjuk ke map_{slot}.pgm')
    ...
    def _manager_command(self, manager, command, timeout=10.0):
        ...
        if not self._wait_client(client, timeout=4.0):
            raise RuntimeError(f'service lifecycle manager tidak tersedia: {manager}')
        request = ManageLifecycleNodes.Request()
        request.command = int(command)
        result = self._future_result(client.call_async(request), timeout=timeout)
        if result is None or not bool(result.success):
            raise RuntimeError(f'{manager} command={command} gagal')
        return True
    ...
    def _activate_navigation(self):
        # Prefer the existing lifecycle managers. If a manager returns failure or
        # times out, recover through each node's canonical lifecycle service and
        # still require every Nav2 runtime node to prove ACTIVE before success.
        warnings = []
        ...
        for node_name in activation_order:
            try:
                self._ensure_node_active(node_name, timeout=30.0)
            except Exception as exc:
    ...
    def _request_nomotion(self):
        if not self._wait_client(self._nomotion, timeout=0.5):
            return False
        try:
            self._future_result(self._nomotion.call_async(Empty.Request()), timeout=2.0)
            return True
        except Exception:
            return False
    ...
    def _tf_ready(self):
        try:
            return (self.tf_buffer.can_transform('map', 'odom', Time(), timeout=Duration(seconds=0.05)) and
                    self.tf_buffer.can_transform('map', 'base_footprint', Time(), timeout=Duration(seconds=0.05)))
        except Exception:
            return False
    ...
    def _initialize_snapshot(self):
        ...
        states = {name: self._node_state(name, timeout=2.0) for name in required}
    ...
    def _start_worker(self, slot, yaml_path):
        try:
            ...
            localized, detail = self._localized()
            ...
            timeout = max(15.0, float(self.get_parameter('localization_timeout_sec').value))
            deadline = time.monotonic() + timeout
            last_nomotion = 0.0
            last_pose_retry = 0.0
            detail = 'waiting'
            while rclpy.ok() and time.monotonic() < deadline:
                ok, detail = self._localized()
                if ok:
                    break
                now = time.monotonic()
            ...
            with self._lock:
                ...
                self._phase = 'ERROR'
                self._error = str(exc)
                self._message = f'Nav2 switch gagal fail-safe: {exc}'
    ...
    def _publish_status(self):
        with self._lock:
            localized, localization_detail = self._localized()
            payload = {
                'busy': self._busy,
                'phase': self._phase,
