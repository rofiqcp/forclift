...
class MappingWebController(Node):
    def __init__(self):
        ...
        self._prewarm_due = time.monotonic() + 4.0
    ...
    def _on_map(self, msg):
        ...
        if self._shared_sensor_mode and self._runtime_alive():
            try:
                tmp = Path(str(LIVE_MAP_PNG) + '.tmp')
                tmp.write_bytes(occupancy_grid_png(msg))
                os.replace(tmp, LIVE_MAP_PNG)
            except Exception as exc:
                self.get_logger().warning(f'Live mapping PNG preview gagal: {exc}')

    def _on_mapping_odom_status(self, msg):
        try:
            payload = json.loads(msg.data)
            if isinstance(payload, dict):
                self._mapping_odom_status = payload
        except Exception:
            pass
    ...
    def _refresh_saved_map_catalog(self, force=False):
        ...
        tmp_catalog = Path(str(SAVED_MAP_CATALOG) + '.tmp')
        tmp_catalog.write_text(json.dumps(payload, separators=(',', ':')), encoding='utf-8')
        os.replace(tmp_catalog, SAVED_MAP_CATALOG)
    ...
    def _delete_service(self, slot, _request, response):
        ...
        if nav_slot == slot:
            response.success = False
            response.message = f'Map {slot} sedang menjadi active map runtime; pindahkan/STOP map aktif terlebih dahulu'
            return response

        pgm = MAP_DIR / f'map_{slot}.pgm'
        yaml_path = MAP_DIR / f'map_{slot}.yaml'
        preview = WEB_STATIC_DIR / f'saved_map_{slot}.png'
        existed = pgm.exists() or yaml_path.exists()
        for path in (pgm, yaml_path, preview):
        ...
        response.message = (f'Map {slot} dihapus: map_{slot}.pgm + map_{slot}.yaml' if existed
    ...
    def _manager_command(self, manager, command):
    ...
    def _pause_navigation(self):
        ...
        paused = []
        # Mapping inside autonomous changes only the two conflicting owners:
        # AMCL owns map->odom and map_server owns /map.  Planner/controller/BT
        # remain untouched; autonomy_health_manager becomes fail-closed as soon
        # as AMCL/map TF are paused, so the already-working Nav2 runtime is not
        # churned merely to perform BAB 4.2 mapping.
        for manager in CRITICAL_MANAGERS:
            service = f'/{manager}/manage_nodes'
            if service not in services:
                raise RuntimeError(f'critical lifecycle manager belum tersedia: {manager}')
            ok, detail = self._manager_command(manager, 1)
            if not ok:
                for previous in reversed(paused):
                    self._manager_command(previous, 2)
                raise RuntimeError(f'gagal pause critical {manager}: {detail}')
            paused.append(manager)
        self._paused_managers = paused
        self.get_logger().info('Mapping isolation READY: paused ' + ', '.join(paused))

    def _enforce_mapping_pause(self):
        if not self._shared_sensor_mode or not self._runtime_alive():
            return
        if all(manager in self._paused_managers for manager in CRITICAL_MANAGERS):
            return
        services = self._service_set()
        for manager in CRITICAL_MANAGERS:
            if manager in self._paused_managers:
                continue
            if f'/{manager}/manage_nodes' not in services:
                self._last_error = f'critical lifecycle manager hilang: {manager}'
                self._phase = 'ERROR'
            ...
            ok, detail = self._manager_command(manager, 1)
            if ok:
                self._paused_managers.append(manager)
                continue
            self._last_error = f'critical pause guard {manager} gagal: {detail}'
            self._phase = 'ERROR'
            self.get_logger().error(self._last_error + '; SLAM dihentikan untuk mencegah konflik TF/map')
            self._stop_process_group()
            self._resume_navigation()
            self._active_slot = 0
            return
    ...
    def _resume_navigation(self):
        ...
        for manager in reversed(self._paused_managers):
            if f'/{manager}/manage_nodes' not in services:
                continue
            ok, detail = self._manager_command(manager, 2)
            if not ok:
        ...
        if errors:
            ...
            self.get_logger().info('Navigation lifecycle RESUMED; autonomous returns to its pre-mapping saved map')
        return errors
    ...
    def _slam_params_path(self):
        configured = str(self.get_parameter('slam_params_file').value or '').strip()
        candidates = [
            Path(configured) if configured else None,
            WORKSPACE / 'config/runtime/navigation/slam_toolbox.yaml',
            WORKSPACE / 'src/navigation/config/slam_toolbox.yaml',
        ]
        params = next((path for path in candidates if path and path.is_file()), None)
        if params is None:
            raise RuntimeError('slam_toolbox.yaml tidak ditemukan')
        return params
    ...
    def _engine_services_ready(self):
        try:
            ...
            return False
        slam_ready = '/slam_toolbox/dynamic_map' in services and '/slam_toolbox/change_state' in services
        bridge_ready = bool(self._mapping_odom_status.get('ready', False))
        return slam_ready and bridge_ready
    ...
    def _engine_housekeeping(self):
        ...
        if self._proc is None:
            if time.monotonic() >= self._prewarm_due:
                try:
                    self._launch_engine('startup-prewarm' if self._engine_started_at <= 0 else 'auto-recover')
                except Exception as exc:
                    self._last_error = f'prewarm mapping engine gagal: {exc}'
                    self._phase = 'ERROR'
                    self._prewarm_due = time.monotonic() + 3.0
            return
        ...
        if (not self._session_active and not self._pending_start_slot and self._engine_ready and
    ...
    def _start_service(self, slot, _request, response):
        ...
        if self._shared_sensor_mode:
            ...
            if self._proc is None or self._proc.poll() is not None:
                try:
                    self._launch_engine('start-request')
                except Exception as exc:
                    self._pending_start_slot = 0
                    self._phase = 'ERROR'
            ...
            if self._engine_ready:
                self._begin_shared_session(slot)
                response.message = f'START MAP {slot}: engine standby READY; scan gate dibuka'
            else:
                response.message = f'START MAP {slot}: request diterima; engine standby sedang finalisasi'
            response.success = True
            self._publish_status()
            return response
        ...
        try:
            ...
            if self._shared_sensor_mode and self._paused_managers:
                self._resume_navigation()
            if self._log_handle:
                try:
                    self._log_handle.close()
    ...
    def _stop_save_service(self, _request, response):
        ...
        self._publish_session_enabled(False)
        self._stop_process_group()
        resume_errors = self._resume_navigation() if self._paused_managers else []
        self._active_slot = 0
        if self._shared_sensor_mode:
            self._phase = 'ENGINE_PREPARING'
            self._prewarm_due = time.monotonic() + 0.25
        ...
        response.success = True
        response.message = (
            f'MAP {slot} PGM tersimpan: {pgm_path}; metadata: {yaml_path}; SLAM dihentikan; navigation di-resume'
    ...
    def _stop_without_save_service(self, _request, response):
        ...
        if self._shared_sensor_mode:
            response.message += '; navigation di-resume'
        if resume_errors:
            response.message += '; resume warning: ' + ' | '.join(resume_errors)
        return response
    ...
    def _publish_status(self):
        ...
        payload = {
            ...
            'navigation_paused': bool(self._paused_managers),
