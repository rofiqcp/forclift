...
class HoleGuidedAlignment:
    ...
    def process_frame(self, canvas, obstacles):
        ...
        # Yaw still comes from the line joining the two pallet holes, but it is
        # accepted only after the center block has also passed hole---block---hole
        # validation. This prevents unrelated blocks from producing alignment data.
        if basic_valid:
        ...
        if basic_valid:
            # Lateral error is measured from the ACTUAL selected center block to
            # the fork/camera setpoint. The midpoint is used only to decide which
            # block is the physical middle block, never as the lateral measurement.
            bx, by = best["cx"], best["cy"]
            m.alignment_center = (bx, by)
            raw_px = bx - self.setpoint_x
