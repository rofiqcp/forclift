...
//------------------------------------------------------------------------
// Struktur controller FOC untuk motor kiri dan kanan.
MotorController motorControllerLeftStorage;                     /* instance controller */
MotorController motorControllerRightStorage;                    /* instance controller */
MotorController *const motorControllerLeft  = &motorControllerLeftStorage;
MotorController *const motorControllerRight = &motorControllerRightStorage;

extern MotorParameters motorParamsLeft;                      /* parameter motor */
MotorControlState       motorStateLeft;                     /* state internal controller */
MotorInput     motorInputLeft;                      /* input controller */
MotorOutput     motorOutputLeft;                      /* output controller */

MotorParameters        motorParamsRight;                     /* parameter motor */
MotorControlState       motorStateRight;                    /* state internal controller */
MotorInput     motorInputRight;                     /* input controller */
MotorOutput     motorOutputRight;                     /* output controller */
...
void MotorSystem_Init(void) {
  ...
  motorParamsLeft.useMeasuredAngle       = 0;            // Motor angle input: 0 = estimated angle, 1 = measured angle (e.g. if encoder is available)
  motorParamsLeft.phaseCurrentMeasurementMode   = 0;            // Left motor measured current phases {Green, Blue} = {iA, iB} -> do NOT change
  motorParamsLeft.controlType         = CTRL_TYP_SEL;
  motorParamsLeft.diagnosticsEnabled            = DIAG_ENA;
  motorParamsLeft.maxCurrent                = (I_MOT_MAX * A2BIT_CONV) << 4;        // fixdt(1,16,4)
  motorParamsLeft.maxSpeed                = N_MOT_MAX << 4;                       // fixdt(1,16,4)
  motorParamsLeft.fieldWeakeningEnabled       = FIELD_WEAK_ENA;
  motorParamsLeft.fieldWeakeningCurrentMax      = (FIELD_WEAK_MAX * A2BIT_CONV) << 4;   // fixdt(1,16,4)
  motorParamsLeft.phaseAdvanceMax          = PHASE_ADV_MAX << 4;                   // fixdt(1,16,4)
  motorParamsLeft.fieldWeakeningHigh        = FIELD_WEAK_HI << 4;                   // fixdt(1,16,4)
  motorParamsLeft.fieldWeakeningLow        = FIELD_WEAK_LO << 4;                   // fixdt(1,16,4)

  motorParamsRight                     = motorParamsLeft;     // Copy the Left motor parameters to the Right motor parameters
  motorParamsRight.phaseCurrentMeasurementMode  = 1;            // Right motor measured current phases {Blue, Yellow} = {iB, iC} -> do NOT change

  /* Masukkan data motor kiri ke struktur input controller FOC. */
  motorControllerLeft->parameters        = &motorParamsLeft;
  motorControllerLeft->state               = &motorStateLeft;
  motorControllerLeft->input              = &motorInputLeft;
  motorControllerLeft->output             = &motorOutputLeft;

  /* Masukkan data motor kanan ke struktur input controller FOC. */
  motorControllerRight->parameters       = &motorParamsRight;
  motorControllerRight->state              = &motorStateRight;
  motorControllerRight->input             = &motorInputRight;
  motorControllerRight->output            = &motorOutputRight;

  /* Initialize BLDC controllers */
  MotorController_Init(motorControllerLeft);
  MotorController_Init(motorControllerRight);
}

/**
 * PENJELASAN InputLimits_Init: menentukan batas command minimum dan maksimum. Saat field weakening aktif rentang mengikuti FIELD_WEAK_HI; pada konfigurasi sekarang tetap -1000 sampai 1000.
 */
void InputLimits_Init(void) {     // Input Limitations - ! Do NOT touch !
  if (motorParamsLeft.fieldWeakeningEnabled || motorParamsRight.fieldWeakeningEnabled) {
...
/**
 * PENJELASAN SerialInput_Init: menginisialisasi USART3 lalu memulai RX circular DMA dan parser frame 64-byte. EEPROM sudah dimuat lebih dahulu oleh RuntimeControl_Init().
 */
...
void calcAvgSpeed(void) {
    // Calculate measured average speed. The minus sign (-) is because motors spin in opposite directions
      speedAvg    = ( motorOutputLeft.motorSpeed - motorOutputRight.motorSpeed) / 2;
...
void updateCurSpdLim(void) {
  ...
  if (input1[0].typ != 0){
    ...
    motorParamsLeft.maxCurrent = motorParamsRight.maxCurrent  = (int16_t)((I_MOT_MAX * A2BIT_CONV * cur_factor) >> 12);    // fixdt(0,16,16) to fixdt(1,16,4)
    cur_spd_valid   = 1;  // Mark update to be saved in Flash at shutdown
  }

  if (input2[0].typ != 0){
    // Update speed limit
    motorParamsLeft.maxSpeed = motorParamsRight.maxSpeed  = (int16_t)((N_MOT_MAX * spd_factor) >> 12);                 // fixdt(0,16,16) to fixdt(1,16,4)
